...
| GET | `/api/stats` | 直近90日のアクセス数（日別・流入元別・端末別） |
| POST | `/api/hit` | アクセスを1回数える。画面の読み込み時に呼ばれる |
...
外部の解析サービスは使わず、自前で数えている。
訪問者のデータを第三者に渡さずに済むのと、
第三者スクリプトを入れるとプライバシーポリシーの設置が要るため。

**個人は追跡しない。** IP もセッション識別子も保存せず、
`page_views` に「日付 × 流入元ホスト × 端末種別」の回数だけを持つ。
1 日あたり数行にしかならないので、無料枠の容量も気にしなくてよい。

流入元は `document.referrer` をクライアントから送っている。
サーバー側の `Referer` ヘッダーは自サイトの URL になるので使えない。
1 タブ 1 回にするため `sessionStorage` に印を付ける。開発ビルドでは数えない。

`/api/stats` に認証は掛けていない。集計値だけで個人情報を含まないため。
見せたくなくなったら環境変数でキーを要求するように変えればよい。
