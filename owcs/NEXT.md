...
最終更新: 2026-09-04（Phase 2 = Korea 全チーム対応まで完了）

## 現在地

**Phase 2（Korea 全チーム）まで完了**。手入力ゼロで、開くだけで情報が揃う状態。

取り込みは**シリーズ軸**。`filter[serie_id]` で 1 リクエスト = 51 試合 = 全チーム分。
...
| やること | 効果 | 重さ |
|---|---|---|
| 試合前の通知 | 見逃さないという目的に直結。HTTPS が整ったので Push が実装可能になった。ただし通知の送信元が必要（スリープするアプリからは送れないので、外部の Cron が要る） | 中 |
| スマホのホーム画面に追加 | 実際に使うための最後の一歩 | 軽 |
| 他地域への拡張 | `OWCS_REGION` を変えるだけで Japan / EMEA / North America 版になる。並べて見たいなら地域切替の UI が要る | 軽〜中 |
